...
def build_model(config, train=True):
    ...
    if model_type == 'swin':
        ...
        if not config.MODEL.PRETRAINED and not config.MODEL.RESUME and not train:
            model.load_from(np.load('/mnt/pretrained_model/imagenet21k_ViT-B_16.npz'))
        ...
        if not config.MODEL.PRETRAINED and not config.MODEL.RESUME and not train:
            pretrained = True
            in_22k = True
        model = eval(model_type)(pretrained=pretrained, in_22k=in_22k, num_classes=config.MODEL.NUM_CLASSES)
    else:
