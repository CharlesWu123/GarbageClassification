...
from data import build_loader, build_loader_test
...
def parse_option():
    ...
    parser.add_argument('--cfg', type=str, required=True,
                        default='configs/garbage/swin_base_patch4_window7_224.yaml',
...
def main(config):
    ...
    with open(os.path.join(config.OUTPUT, 'class_to_idx.json'), 'w', encoding='utf-8') as f:
        f.write(json.dumps(dataset_train.class_to_idx, indent=2, ensure_ascii=False))
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        acc1, acc5, loss = validate(config, data_loader_val, model)
        writer.add_scalar('val/loss', loss, (epoch + 1) * len(data_loader_train))
        writer.add_scalar('val/acc', acc1, (epoch + 1) * len(data_loader_train))
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
    ...
    writer.close()
    dataset_test, data_loader_test = build_loader_test(config, dataset_train.class_to_idx)
    test(config, data_loader_test, model, dataset_train.classes)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        writer.add_scalar('train/loss', loss_meter.avg, epoch * num_steps + idx)
        writer.add_scalar('train/lr', lr, epoch * num_steps + idx)
        # writer.add_scalar('train/acc', acc_meter.avg, epoch * num_steps + idx)
